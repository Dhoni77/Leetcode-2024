//
// Created by Aldrin on 12-02-2024.
//

#ifndef DOOCS_MAJORITYELEMENT_H
#define DOOCS_MAJORITYELEMENT_H
#include "bits/stdc++.h"
using namespace std;
...
public:
     ...
     *
     * int majorityElement(vector<int>& nums) {
        int majorityEle = 0, n = nums.size();
        for(int i=0;i<32;i++) {
            int bit = 1 << i, bitCount = 0;
            for(int num: nums) {
                if(num & bit) {
                    bitCount++;
                }
     ...
     */
    int majorityElement(vector<int>& nums) {
        int majorityEle = 0, n = nums.size(), count = 0;
        for(int i=0;i<n;i++) {
            if(count == 0) {
                majorityEle = nums[i];
            }
